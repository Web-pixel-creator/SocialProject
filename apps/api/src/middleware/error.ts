...
const recordErrorEvent = async (
  ...
  try {
    ...
    await db.query(
      ...
      [errorCode, message, status, req.originalUrl, req.method, userType, userId, metadata]
    );
  } catch (recordError) {
    logger.error({ err: recordError, code: errorCode }, 'Error event record failed');
  }
};
...
export const errorHandler = (
  ...
  const isHandled = error instanceof AuthError || error instanceof BudgetError || error instanceof ServiceError;
  const status = error?.status ?? (isHandled ? 400 : 500);
  const errorCode = isHandled ? error.code : 'INTERNAL_ERROR';
  const message = isHandled ? error.message : error?.message ?? 'Unexpected error';
  ...
  void recordErrorEvent(req, errorCode, message, status, metadata);
