...
const SEARCH_INTENTS: SearchIntent[] = ['needs_help', 'seeking_pr', 'ready_for_review'];
const VISUAL_TYPES: NonNullable<VisualSearchFilters['type']>[] = ['draft', 'release', 'all'];
const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const VISUAL_ALLOWED_FIELDS = new Set(['embedding', 'draftId', 'type', 'tags', 'limit', 'offset']);
...
const parseLimit = (
  ...
  if (!(Number.isFinite(parsed) && Number.isInteger(parsed))) {
    throw new ServiceError('SEARCH_PAGINATION_INVALID', `${field} must be an integer.`, 400);
  }
...
const parseOffset = (
  value: unknown,
  { field = 'offset', max = SEARCH_MAX_OFFSET }: { field?: string; max?: number } = {},
): number | undefined => {
  if (value === undefined || value === null || value === '') {
    return undefined;
  }
  ...
  if (!(Number.isFinite(parsed) && Number.isInteger(parsed)) || parsed < 0) {
    throw new ServiceError('SEARCH_PAGINATION_INVALID', `${field} must be an integer >= 0.`, 400);
  }
  if (parsed > max) {
    throw new ServiceError(
      'SEARCH_PAGINATION_INVALID',
...
const assertAllowedFields = (
  ...
  if (unknown.length > 0) {
    throw new ServiceError(errorCode, `Unsupported fields: ${unknown.join(', ')}.`, 400);
  }
};
...
const assertAllowedQueryFields = (
  ...
  const unknown = Object.keys(queryRecord).filter((key) => !allowedSet.has(key));
  if (unknown.length > 0) {
  ...
  async (req, res, next) => {
    try {
      const query = assertAllowedQueryFields(req.query, {
        allowed: ['q', 'type', 'sort', 'range', 'profile', 'intent', 'limit', 'offset'],
  ...
  async (req, res, next) => {
    try {
      ...
      if (excludeDraftId && !isUuid(excludeDraftId)) {
        throw new ServiceError('DRAFT_ID_INVALID', 'Invalid exclude draft id.', 400);
      }
...
router.post('/search/visual', computeHeavyRateLimiter, async (req, res, next) => {
  try {
    ...
    assertAllowedFields(body, VISUAL_ALLOWED_FIELDS, 'SEARCH_VISUAL_INVALID_FIELDS');
    const { embedding, draftId, tags, limit, offset } = body;
    const normalizedDraftId = typeof draftId === 'string' ? draftId.trim() : undefined;
    ...
    if (embedding !== undefined) {
      if (!Array.isArray(embedding)) {
        throw new ServiceError('EMBEDDING_INVALID', 'embedding must be a numeric array.', 400);
      }
      parsedEmbedding = embedding.map((value) => Number(value));
      if (!parsedEmbedding.every((value) => Number.isFinite(value))) {
        throw new ServiceError('EMBEDDING_INVALID', 'embedding must contain only numbers.', 400);
      }
    }
...
router.post('/search/style-fusion', computeHeavyRateLimiter, async (req, res, next) => {
  let draftId = '';
  ...
  try {
    ...
    const body =
      req.body && typeof req.body === 'object' ? (req.body as Record<string, unknown>) : {};
    assertAllowedFields(body, STYLE_FUSION_ALLOWED_FIELDS, 'STYLE_FUSION_INVALID_FIELDS');
    ...
    if (draftId && isUuid(draftId)) {
      const errorCode = error instanceof ServiceError ? error.code : 'STYLE_FUSION_INTERNAL_ERROR';
      await writeStyleFusionTelemetry({
        draftId,
        status: 'error',
        metadata: {
