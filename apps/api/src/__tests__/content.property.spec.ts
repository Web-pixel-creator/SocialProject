...
describe('content generation properties', () => {
  ...
  test('Property 50: Daily GlowUp Reel Selection', async () => {
    ...
    try {
      ...
      await client.query(
        ...
        [recent.rows[0].id, 'https://example.com/recent-v1.png', 'https://example.com/recent-v1-thumb.png', authorId]
      );
      await client.query(
        'INSERT INTO versions (draft_id, version_number, image_url, thumbnail_url, created_by) VALUES ($1, 2, $2, $3, $4)',
        [recent.rows[0].id, 'https://example.com/recent-v2.png', 'https://example.com/recent-v2-thumb.png', authorId]
      );
      await client.query(
        'INSERT INTO versions (draft_id, version_number, image_url, thumbnail_url, created_by) VALUES ($1, 1, $2, $3, $4)',
        [old.rows[0].id, 'https://example.com/old-v1.png', 'https://example.com/old-v1-thumb.png', authorId]
      );
  ...
  test('Property 51: GlowUp Reel Credits', async () => {
    ...
    try {
      ...
      await client.query(
        'INSERT INTO versions (draft_id, version_number, image_url, thumbnail_url, created_by) VALUES ($1, 1, $2, $3, $4)',
        [draft.rows[0].id, 'https://example.com/v1.png', 'https://example.com/v1-thumb.png', author.rows[0].id]
      );
      await client.query(
        'INSERT INTO versions (draft_id, version_number, image_url, thumbnail_url, created_by) VALUES ($1, 2, $2, $3, $4)',
        [draft.rows[0].id, 'https://example.com/v2.png', 'https://example.com/v2-thumb.png', maker.rows[0].id]
      );
      ...
      expect(reel.items[0].credits.makers.map((item) => item.id)).toContain(maker.rows[0].id);
