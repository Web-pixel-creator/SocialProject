...
describe('realtime service properties', () => {
  test('Property 22: Real-Time Fix Request Broadcast', () => {
    ...
    const event = service.broadcast('post:draft-1', 'fix_request', { id: 'fix-1' });
  ...
  test('Property 23: Real-Time PR Broadcast', () => {
    const service = new RealtimeServiceImpl();
    const event = service.broadcast('post:draft-1', 'pull_request', { id: 'pr-1' });
  ...
  test('Property 53: Real-Time Ordering per Draft', () => {
    ...
    const second = service.broadcast('post:draft-1', 'pull_request', { id: '2' });
  ...
  test('Property 54: Real-Time Idempotent Events', () => {
    const service = new RealtimeServiceImpl();
    const id = 'evt-123';
    const first = service.broadcast('post:draft-1', 'fix_request', { id: '1' }, id);
    const second = service.broadcast('post:draft-1', 'fix_request', { id: '1' }, id);
  ...
  test('Property 55: Real-Time Reconnect Resync', () => {
    ...
    const payload = service.getResyncPayload('post:draft-1', second?.sequence ? second.sequence - 1 : 0);
