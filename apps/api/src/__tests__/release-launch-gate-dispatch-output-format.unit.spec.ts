...
describe('launch-gate dispatch summary output formatter', () => {
  test('renders baseline lines without optional flags', () => {
    const result = runBuildSummary({
      ...
      githubApiRetryBackoffFactor: 2,
      githubApiRetryDelayMs: 2000,
      githubApiRetryJitterPercent: 20,
      githubApiRetryMaxAttempts: 3,
      githubApiRetryMaxDelayMs: 10_000,
      ...
      runtimeDraftId: '',
      smokeTimeoutRetries: null,
      smokeTimeoutRetryDelayMs: null,
    ...
    expect(result.payload.result).toEqual([
      ...
      'Smoke timeout retries input: default',
      'Smoke timeout retry delay ms input: default',
      'GitHub API retry max attempts: 3',
      'GitHub API retry base delay ms: 2000',
      'GitHub API retry backoff factor: 2',
      'GitHub API retry max delay ms: 10000',
      'GitHub API retry jitter percent: 20',
  ...
  test('renders optional lines in stable order when options are enabled', () => {
    const result = runBuildSummary({
      ...
      githubApiRetryBackoffFactor: 3,
      githubApiRetryDelayMs: 1500,
      githubApiRetryJitterPercent: 0,
      githubApiRetryMaxAttempts: 4,
      githubApiRetryMaxDelayMs: 12_000,
      ...
      smokeTimeoutRetries: 2,
      smokeTimeoutRetryDelayMs: 7000,
    ...
    expect(result.payload.result).toEqual([
      ...
      'Require inline health artifacts input: true',
      'Smoke timeout retries input: 2',
      'Smoke timeout retry delay ms input: 7000',
      'GitHub API retry max attempts: 4',
      'GitHub API retry base delay ms: 1500',
      'GitHub API retry backoff factor: 3',
      'GitHub API retry max delay ms: 12000',
      'GitHub API retry jitter percent: 0',
