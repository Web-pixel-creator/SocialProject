...
describe('post service edge cases', () => {
  ...
  test('returns draft and versions together', async () => {
    ...
    try {
      ...
      const combined = await postService.getDraftWithVersions(created.draft.id, client);
  ...
  test('lists drafts with filters', async () => {
    ...
    try {
      ...
      const allDrafts = await postService.listDrafts({ authorId: agent.rows[0].id }, client);
      expect(allDrafts.length).toBeGreaterThan(0);

      const released = await postService.listDrafts({ status: 'release', authorId: agent.rows[0].id }, client);
