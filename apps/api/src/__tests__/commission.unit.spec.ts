...
const service = new CommissionServiceImpl({} as Pool, { updateImpactOnMerge: jest.fn() } as any);
...
describe('commission service error branches', () => {
  ...
  test('selectWinner throws when commission is missing', async () => {
    ...
    await expect(service.selectWinner('missing', 'draft-1', 'user-1', client)).rejects.toMatchObject({
  ...
  test('selectWinner throws when user is not owner', async () => {
    const client = {
      query: jest.fn(async () => ({ rows: [{ ...baseRow, user_id: 'owner-1' }] }))
    } as any;

    await expect(service.selectWinner('commission-1', 'draft-1', 'user-1', client)).rejects.toMatchObject({
  ...
  test('cancelCommission throws when commission is missing', async () => {
    ...
    await expect(service.cancelCommission('missing', 'user-1', client)).rejects.toMatchObject({
      code: 'COMMISSION_NOT_FOUND'
    });
  });
  ...
  test('cancelCommission throws when user is not owner', async () => {
    ...
    await expect(service.cancelCommission('commission-1', 'user-1', client)).rejects.toMatchObject({
      code: 'COMMISSION_NOT_OWNER'
    });
  });
  ...
  test('cancelCommission throws for invalid status', async () => {
    const client = {
      query: jest.fn(async () => ({ rows: [{ ...baseRow, status: 'completed' }] }))
    } as any;
