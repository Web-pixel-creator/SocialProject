...
describe('payment service unit', () => {
  ...
  test('createPaymentIntent throws for missing commission', async () => {
    ...
    await expect(service.createPaymentIntent(missingCommissionId)).rejects.toThrow(/Commission not found/);
  });

  test('markEscrowed throws for missing commission', async () => {
    const service = new PaymentServiceImpl(pool);
    await expect(service.markEscrowed(missingCommissionId)).rejects.toThrow(/Commission not found/);
  });

  test('payoutWinner throws for missing commission', async () => {
    const service = new PaymentServiceImpl(pool);
    await expect(service.payoutWinner(missingCommissionId)).rejects.toThrow(/Commission not found/);
  });

  test('refundCommission throws for missing commission', async () => {
    const service = new PaymentServiceImpl(pool);
    await expect(service.refundCommission(missingCommissionId)).rejects.toThrow(/Commission not found/);
  });
});
