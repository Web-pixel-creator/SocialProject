...
describe('privacy service edge cases', () => {
  ...
  test('builds export bundle with profile fallback', async () => {
    ...
    const client = {
      query: jest.fn()
        ...
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [{ draft_id: 'draft-1', viewed_at: createdAt }] })
        .mockResolvedValueOnce({ rows: [{ id: 'commission-1', status: 'open', reward_amount: 10 }] })
    } as any;
  ...
  test('getExportStatus throws when export missing', async () => {
    ...
    await expect(service.getExportStatus('missing', client)).rejects.toMatchObject({
  ...
  test('rejects deletion requests when another is pending', async () => {
    ...
    await expect(service.requestDeletion('user-1', client)).rejects.toMatchObject({
  ...
  test('completes deletion and anonymizes user', async () => {
    ...
    const client = {
      query: jest.fn()
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [{ id: 'del-1', user_id: 'user-1', status: 'pending', requested_at: requestedAt }] })
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({
          rows: [
            {
