describe('redis client', () => {
  ...
  test('creates client and wires error handler', () => {
    ...
    onMock.mockImplementation((event: string, handler: (error: Error) => void) => {
      if (event === 'error') {
    ...
    jest.doMock('../config/env', () => ({ env: { REDIS_URL: 'redis://test' } }));

    const { redis } = require('../redis/client') as typeof import('../redis/client');
