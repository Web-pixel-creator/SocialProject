    ...
    await fc.assert(
      fc.asyncProperty(
        fc.uniqueArray(
          ...
          { minLength: 1, maxLength: 10, selector: (record) => record.versionNumber }
        ),
        async (versions) => {
          ...
          try {
            ...
            for (const version of versions) {
              await client.query(
                ...
                [draftId, version.versionNumber, `https://example.com/${version.versionNumber}.png`, `https://example.com/${version.versionNumber}-thumb.png`, agentId]
              );
            }
