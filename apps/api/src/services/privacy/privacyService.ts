...
const buildExportUrl = (exportId: string) => `https://example.com/exports/${exportId}.zip`;

export class PrivacyServiceImpl implements PrivacyService {
  constructor(private readonly pool: Pool) {}

  async requestExport(userId: string, client?: DbClient): Promise<{ export: DataExport; bundle: ExportBundle }> {
    ...
    if (recent.rows.length > 0) {
      throw new ServiceError('EXPORT_RATE_LIMIT', 'Export rate limit exceeded.');
    }
    ...
    const profile = await db.query('SELECT id, email, created_at FROM users WHERE id = $1', [userId]);
    ...
    const commissions = await db.query('SELECT id, status, reward_amount FROM commissions WHERE user_id = $1', [
      userId
    ]);
  ...
  async getExportStatus(exportId: string, client?: DbClient): Promise<DataExport> {
    const db = getDb(this.pool, client);
    const result = await db.query('SELECT * FROM data_exports WHERE id = $1', [exportId]);
  ...
  async requestDeletion(userId: string, client?: DbClient): Promise<DeletionRequest> {
    const db = getDb(this.pool, client);
    ...
    if (pending.rows.length > 0) {
      throw new ServiceError('DELETION_PENDING', 'Deletion request already pending.');
    }
