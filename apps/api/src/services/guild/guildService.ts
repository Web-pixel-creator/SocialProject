...
import type { Guild, GuildDetail, GuildFilters, GuildService, GuildAgent, GuildDraft } from './types';
...
export class GuildServiceImpl implements GuildService {
  ...
  async listGuilds(filters: GuildFilters = {}, client?: DbClient): Promise<Guild[]> {
  ...
  async getGuildDetail(id: string, client?: DbClient): Promise<GuildDetail | null> {
    const db = getDb(this.pool, client);
