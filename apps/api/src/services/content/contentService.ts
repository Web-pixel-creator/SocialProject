...
import type { AutopsyPattern, AutopsyReport, ContentGenerationService, GlowUpReel, GlowUpReelItem } from './types';
...
export class ContentGenerationServiceImpl implements ContentGenerationService {
  ...
  async generateGlowUpReel(limit = 5, client?: DbClient): Promise<GlowUpReel> {
    ...
    for (const row of drafts.rows) {
      ...
      const after = versions.rows[versions.rows.length - 1];

      const author = await db.query('SELECT id, studio_name FROM agents WHERE id = $1', [row.author_id]);
  ...
  async generateAutopsyReport(limit = 5, client?: DbClient): Promise<AutopsyReport> {
    ...
    for (const row of drafts.rows) {
      const draftId = row.id as string;
      const fixCount = await db.query('SELECT COUNT(*) FROM fix_requests WHERE draft_id = $1', [draftId]);
      const rejectCount = await db.query(
        "SELECT COUNT(*) FROM pull_requests WHERE draft_id = $1 AND status = 'rejected'",
        [draftId]
      );
      const prCount = await db.query('SELECT COUNT(*) FROM pull_requests WHERE draft_id = $1', [draftId]);
      ...
      if (rejected >= Math.max(1, Math.floor(totalPr / 2))) highRejectCount += 1;
    ...
    if (budgetExhaustedCount > 0) summaryParts.push('budget exhaustion patterns');
