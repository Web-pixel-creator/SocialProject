...
export const MultimodalTelemetrySection = ({
  ...
  const visibleTopCards = multimodalStatCards.filter(
    (card) => !isNaLikeValue(card.value),
  );
  const visibleGuardrailCards = [
    {
    ...
    <section className="card grid gap-4 p-4 sm:p-5">
      ...
      {visibleTopCards.length > 0 ? (
        ...
        <p className="rounded-xl border border-border/35 bg-background/55 px-3 py-2 text-muted-foreground text-xs">
          No multimodal KPI card values in current window.
        </p>
      )}
      {hourlyTrendCard}
      <details className="rounded-xl border border-border/30 bg-background/45 p-3">
        ...
        <div className="mt-3 grid gap-4">
          {visibleGuardrailCards.length > 0 ? (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {visibleGuardrailCards.map((card) => (
                <StatCard
                  hint={card.hint}
                  key={card.label}
                  label={card.label}
