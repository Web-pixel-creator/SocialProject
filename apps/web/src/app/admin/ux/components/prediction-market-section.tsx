...
export const PredictionMarketSection = ({
  ...
  const visibleTopCards = predictionStatCards.filter(
    (card) => !isNaLikeValue(card.value),
  );
  const visibleShareCards = [
    {
  ...
  return (
    <section className="card grid gap-4 p-4 sm:p-5">
      ...
      {visibleTopCards.length > 0 ? (
        ...
        <CompactEmptyState message="No prediction KPI cards with usable values in current window." />
      )}
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <article className="card grid gap-2 p-4">
          ...
          <p className="text-muted-foreground text-xs">
            ...
            {windowThresholdCriticalText} | min sample:{' '}
            {windowThresholdMinSample}
          </p>
          <p className="text-muted-foreground text-xs">
            {window7d.days}d:{' '}
            ...
            ({window7d.correctPredictions}/{window7d.resolvedPredictions}) |
            Net:{' '}
            <span className="font-semibold text-foreground">
