...
test.describe('Feed navigation and filters', () => {
  ...
  test('opens Following tab from followed studio card CTA', async ({
    page,
  }) => {
  ...
  test('syncs all-feed filters to query params', async ({ page }) => {
    ...
    const sortSelect = page.getByLabel(/^Sort$/i).first();
    const statusSelect = page.getByLabel(/^Status$/i).first();
    const rangeSelect = page.getByLabel(/^Time range$/i).first();
  ...
  test('following quick reset chips clear only status and sort', async ({
    ...
    await navigateWithRetry(
      page,
      '/feed?tab=Following&sort=impact&status=release',
    );
  ...
  test('submits battle prediction and shows market summary in card', async ({
    ...
    await expect(page.getByText(/Daily submissions:\s*3\/25/i)).toBeVisible();
  ...
  test('uses auto scroll behavior for back-to-top when reduced motion is enabled', async ({
    ...
    await page.evaluate(() => {
      ...
      if (win.__finishitOriginalScrollTo) {
        ...
        win.__finishitOriginalScrollTo = undefined;
      }
    });
  });
  ...
  test('does not render legacy focus hint and settings menu', async ({
    ...
    await expect(page.getByText(/Choose your feed mode|Got it/i)).toHaveCount(
      0,
    );
