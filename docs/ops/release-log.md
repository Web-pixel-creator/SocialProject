...
- Changes:
  - Added test suite:
    ...
    - covers:
      ...
      - `readTokenFromGitCredentialStore` with isolated git credential helper fixture:
        - valid token path,
        - empty-password path with `allowMissing: true` (returns empty),
        - empty-password path with `allowMissing: false` (throws expected error).
- Validation:
...
- Incidents:
  - none.
- Follow-ups:
  - optional: add integration-style coverage for `resolveRepoSlug` fallback via isolated temporary git remote fixture.
...
- Release commander: Codex automation.
- Window (UTC): 2026-03-05 15:39 -> 2026-03-05 15:42.
- Changes:
  - Updated `scripts/release/dispatch-staging-smoke-tunnel.mjs`:
    - replaced local JSON `githubRequest` implementation with `githubApiRequestWithTransientRetry`,
    - retained existing text-log download path (`githubRequestText`) unchanged,
    - added retry label context (`[release:smoke:dispatch:tunnel] ...`) for transient diagnostics.
- Validation:
  - `node --check scripts/release/dispatch-staging-smoke-tunnel.mjs`: pass.
  - `npm run ci:workflow:inline-node-check`: pass.
  - `npm run lint`: pass.
  - `npm run ultracite:check`: pass.
  - `npm run release:runbook:failure-snippet:check`: pass.
  - `npm run release:launch:gate:production:json -- --required-external-channels all`: pass (`status: pass`, `generatedAtUtc: 2026-03-05T15:42:20.641Z`).
- Incidents:
  - none.
- Follow-ups:
  - optional: migrate `collect-retry-failure-logs.mjs` to shared helper to complete this migration stream.
...
- Release commander: Codex automation.
- Window (UTC): 2026-03-05 13:27 -> 2026-03-05 13:31.
- Changes:
  - Updated `.github/workflows/ci.yml`:
    - assigned id `runbook_snippet_parity` to parity step,
    - added conditional summary step:
      - runs with `if: always() && steps.runbook_snippet_parity.outcome == 'failure'`,
      - appends quick troubleshooting block to `$GITHUB_STEP_SUMMARY` with command + fixture + runbook paths.
- Validation:
  - `npm run ci:workflow:inline-node-check`: pass.
  - `npm run release:runbook:failure-snippet:check`: pass.
  - `npx jest --runInBand apps/api/src/__tests__/release-launch-gate-production-failure-runbook-check-script.unit.spec.ts apps/api/src/__tests__/release-launch-gate-production-failure-runbook-example.unit.spec.ts --config jest.config.cjs`: pass.
  - `npm run lint`: pass.
  - `npm run ultracite:check`: pass.
  - `npm run release:launch:gate:production:json -- --required-external-channels all`: pass (`status: pass`, `generatedAtUtc: 2026-03-05T13:29:33.277Z`).
  - live dispatch regression:
    - `npm run release:launch:gate:dispatch -- --required-external-channels all --require-inline-health-artifacts --print-artifact-links --artifact-link-names all`
    - run `#95` (`22720256211`): success with expected summary and artifact-link output.
- Incidents:
  - none.
- Follow-ups:
  - optional: emit a compact expected-vs-actual first-diff line in CI summary when parity mismatch occurs.
...
- Release commander: Codex automation.
- Window (UTC): 2026-03-05 12:29 -> 2026-03-05 12:33.
- Changes:
  - Updated `scripts/release/dispatch-production-launch-gate-failure-summary.mjs`:
    - added `pickFirstFailedJobLogUrl`,
    - appends `First failed job logs: <url>` when at least one failed job exposes `html_url`,
    - preserves previous summary behavior when no failed-job URL is available.
  - Updated `apps/api/src/__tests__/release-launch-gate-dispatch-failure-summary.unit.spec.ts`:
    - extended existing failed-job rendering expectation with URL hint,
    - added coverage for skipping blank `html_url` and selecting first non-empty failed-job URL.
- Validation:
  - `npx jest --runInBand apps/api/src/__tests__/release-launch-gate-dispatch-failure-summary.unit.spec.ts apps/api/src/__tests__/release-launch-gate-dispatch-cli-args.unit.spec.ts apps/api/src/__tests__/release-launch-gate-dispatch-link-options.unit.spec.ts apps/api/src/__tests__/release-launch-gate-dispatch-output-format.unit.spec.ts apps/api/src/__tests__/release-launch-gate-dispatch-token-resolution.unit.spec.ts apps/api/src/__tests__/release-launch-gate-dispatch-help-snapshot.unit.spec.ts apps/api/src/__tests__/release-launch-gate-dispatch-external-channels.unit.spec.ts apps/api/src/__tests__/release-launch-gate-production-cli-args.unit.spec.ts apps/api/src/__tests__/release-production-config-resolvers.unit.spec.ts apps/api/src/__tests__/release-env-parse-utils.unit.spec.ts --config jest.config.cjs`: pass.
  - `npm run lint`: pass.
  - `npm run ultracite:check`: pass.
  - `npm run ci:workflow:inline-node-check`: pass.
  - `npm run release:launch:gate:production:json -- --required-external-channels all`: pass (`status: pass`, `generatedAtUtc: 2026-03-05T12:31:27.327Z`).
  - live dispatch regression:
    - `npm run release:launch:gate:dispatch -- --required-external-channels all --require-inline-health-artifacts --print-artifact-links --artifact-link-names all`
    - run `#84` (`22718041626`): success with expected summary and artifact-link output.
- Incidents:
  - none.
- Follow-ups:
  - optional: include per-failed-job log URL hints (not just first available) when we need richer multi-failure triage context.
