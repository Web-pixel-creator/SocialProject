...
const resolveArtifactPaths = (runId) =>
  REQUIRED_ARTIFACT_RELATIVE_PATHS.map((template) =>
    template.replace(/<run_id>/gu, String(runId)),
  );
...
const main = async () => {
  ...
  const checks = await Promise.all(requiredPaths.map((relativePath) => checkArtifact(relativePath)));
